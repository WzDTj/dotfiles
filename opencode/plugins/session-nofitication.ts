...
async function hasIncompleteTodos(ctx: PluginInput, sessionID: string): Promise<boolean> {
  if (typeof ctx.client.session.todo !== "function") return false
...
function getEventToolName(properties: Record<string, unknown> | undefined): string | undefined {
...
function createIdleScheduler(options: {
  ctx: PluginInput
  config: Required<TaskIdleTTSConfig>
  ...
  function markSessionActivity(sessionID: string): void {
    ...
    if (
      options.config.activityGracePeriodMs > 0
      && scheduledTime !== undefined
      && Date.now() - scheduledTime <= options.config.activityGracePeriodMs
    ) {
      return
    }
  ...
  async function executeNotification(sessionID: string, version: number): Promise<void> {
    ...
    try {
      if (options.config.skipIfIncompleteTodos) {
        const hasPendingWork = await hasIncompleteTodos(options.ctx, sessionID)
        if (notificationVersions.get(sessionID) !== version) {
          return
        }
...
export const SessionIdleTTS: Plugin = async (ctx) => {
  const mergedConfig: Required<TaskIdleTTSConfig> = {
  ...
  const shouldNotifyForSession = (sessionID: string): boolean => {
    ...
    if (mergedConfig.enforceMainSessionFilter && mainSessionID && sessionID !== mainSessionID) {
      return false
    }
